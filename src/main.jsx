  ...
  useEffect(() => {
    ...
    utils.file.loadJSON("/data/settings.json").then((response) => {
      ...
      if (consoleMessageForDevelopers) {
        ...
        utils.log.info(
          consoleMessageForDevelopers.title,
          consoleMessageForDevelopers.items,
          primaryColor
        );
      }
    });
...
const AppCapabilitiesWrapper = ({ children }) => {
  ...
  const animatedCursorEnabled =
    appSettings["templateSettings"].animatedCursorEnabled;
  const showSpinnerOnThemeChange =
    appSettings["templateSettings"].showSpinnerOnThemeChange;
